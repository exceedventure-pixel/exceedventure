...
/**
 * Password sign-in for the client dashboard.
 *
 * Payload's own /api/client-accounts/login would work, but it sets the shared
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const maxAge = payload.collections['client-accounts']!.config.auth.tokenExpiration ?? 7200
    const cookieName = cookieNameFor('client-accounts')
